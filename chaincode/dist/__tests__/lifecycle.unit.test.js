...
describe('Create commit with normal events', () => {
    ...
    it('should createCommit', async () => cc.createCommit(context, entityName, id, version, JSON.stringify(events), commitId)
        .then((response) => JSON.parse(response)[commitId])
...
describe('Create commit with lifecycle starting events', () => {
    ...
    it('should reject create', async () => {
        ...
        return cc.createCommit(context, entityName, id, version, JSON.stringify(events), commitId)
            .catch((error) => expect(error.message).toBe(`Lifecycle of ${id} already started`));
    });
});
describe('Create commit with lifecycle ending events', () => {
    ...
    it('should reject create', async () => {
        expect.assertions(1);
        return cc.createCommit(context, entityName, id, version, JSON.stringify(events), commitId)
            .catch((error) => expect(error.message).toBe(`Lifecycle of ${id} already ended`));
    });
});
describe('Create commit with lifecycle starting and ending events', () => {
    ...
    it('should create commit', async () => {
        expect.assertions(1);
        return cc.createCommit(context, entityName, id, version, JSON.stringify(events), commitId)
            .catch((error) => expect(error.message).toBe(`Lifecycle of ${id} not started yet`));
    });
});
describe('Create commit with lifecycle ending then starting events', () => {
    ...
    it('should create commit', async () => {
        expect.assertions(1);
        return cc.createCommit(context, entityName, id, version, JSON.stringify(events), commitId)
            .catch((error) => expect(error.message).toBe(`Cannot end ${id} before starting`));
    });
});
